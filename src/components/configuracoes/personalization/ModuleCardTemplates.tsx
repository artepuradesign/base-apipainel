...
const ModuleCardTemplates = ({ module, template, theme }: ModuleCardTemplatesProps) => {
  ...
  if (template === 'corporate') {
    return (
      <Card className={`w-[130px] h-[180px] ${styles.card} relative overflow-hidden`}>
        <CardContent className="p-2 h-full flex flex-col">
          ...
          <div className="absolute top-2 right-2 text-right z-10">
            {module.originalPrice && module.discountPercentage ? (
              <div className="flex flex-col space-y-0.5">
                ...
                <div className="text-[10px] text-gray-500 dark:text-gray-400 line-through">
                  {module.originalPrice}
                </div>
                <div className="text-[9px] bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-1 py-0.5 rounded-full text-center">
                  -{module.discountPercentage}%
                </div>
              </div>
            ) : (
              <div className={`text-xs font-bold ${styles.icon}`}>
                {module.price}
              </div>
            )}
          </div>
          ...
          <div className="absolute top-2 left-2 z-10">
            <div className={`p-1 ${styles.iconBg} rounded-lg border`}>
              <ModuleIcon className={`h-8 w-8 ${styles.icon}`} />
            </div>
          </div>
          <div className="flex-grow flex flex-col justify-between pt-12">
            <div className="flex-grow">
              <h3 className={`font-semibold text-xs ${styles.text} mb-1 mt-1 truncate`}>{module.title}</h3>
              <p className={`text-[10px] ${styles.textMuted} line-clamp-2 mb-1`}>
                {module.description}
              </p>
            </div>
            <Button className={`w-full text-[10px] h-6 ${styles.button}`}>
              Acessar
            </Button>
          </div>
        </CardContent>
        ...
        <CardContent className="p-2 h-full flex flex-col relative overflow-hidden">
          ...
          <div className="absolute top-2 right-2 text-right z-20">
            {module.originalPrice && module.discountPercentage ? (
              <div className="flex flex-col space-y-0.5">
                ...
                <div className="text-[9px] bg-white/20 text-white px-1 py-0.5 rounded-full text-center">
                  -{module.discountPercentage}%
                </div>
              </div>
            ) : (
          ...
          <div className="absolute top-2 left-2 z-20">
            <div className={`p-1 ${styles.iconBg} rounded-lg`}>
              <ModuleIcon className="h-8 w-8" style={{ color: 'white' }} />
            </div>
          </div>
          <div className="relative z-10 h-full flex flex-col pt-12">
            <div className="flex-grow flex flex-col justify-between">
              <div className="flex-grow">
                <h3 className="text-xs font-bold mb-1 mt-1 truncate">{module.title}</h3>
                <p className="text-[10px] text-white/90 mb-1 line-clamp-2">
                  {module.description}
                </p>
              </div>
              <Button className={`w-full text-[10px] h-6 ${styles.button} group`}>
                <span>Acessar</span>
                <ArrowRight className="h-2 w-2 ml-1 group-hover:translate-x-1 transition-transform" />
              </Button>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent className="p-2 h-full flex flex-col relative">
          ...
          <div className="flex-grow flex flex-col justify-between pt-12">
            ...
            <Button 
              size="sm" 
              variant="outline" 
              className={`w-full text-[10px] h-6 ${styles.button}`}
            >
              Acessar
            </Button>
          </div>
      ...
      <Card className={`w-[130px] h-[180px] ${styles.card} relative overflow-hidden`}>
        <CardContent className="p-2 h-full flex flex-col relative">
          ...
          <div className="relative z-10 h-full flex flex-col pt-12">
            <div className="flex-grow flex flex-col justify-between">
              <div className="flex-grow">
                <h3 className={`font-semibold text-xs ${styles.text} mb-1 mt-1 truncate`}>
                  {module.title}
                </h3>
                <p className={`text-[10px] ${styles.textMuted} line-clamp-2 mb-1`}>
                  {module.description}
  ...
  if (template === 'matrix') {
    return (
      <Card className={`w-[130px] h-[180px] ${styles.card} relative overflow-hidden`}>
        <CardContent className="p-2 h-full flex flex-col relative font-mono bg-black">
          ...
          <div className="absolute top-2 right-2 text-right z-20">
            {module.originalPrice && module.discountPercentage ? (
              <div className="flex flex-col space-y-0.5">
                ...
                <div className="text-[10px] text-green-300/70 line-through font-mono">
                  {module.originalPrice}
                </div>
                <div className="text-[9px] bg-green-400/20 text-green-400 px-1 py-0.5 rounded text-center font-mono">
                  -{module.discountPercentage}%
                </div>
              </div>
            ) : (
          ...
          <div className="relative z-10 h-full flex flex-col pt-12">
            <div className="flex-grow flex flex-col justify-between">
              <div className="flex-grow">
                <h3 className={`font-semibold text-xs ${styles.text} mb-1 mt-1 truncate font-mono`}>
                  {module.title.toUpperCase().replace(/\s+/g, '_')}.EXE
                </h3>
                <p className={`text-[10px] ${styles.textMuted} line-clamp-2 mb-1 font-mono`}>
                  // {module.description}
                </p>
              </div>
              <Button className={`w-full text-[10px] h-6 ${styles.button} font-mono`}>
                {'>'} ACESSAR
              </Button>
            </div>
          </div>
  ...
  return (
    <Card className={`w-[130px] h-[180px] ${styles.card} relative group overflow-hidden`}>
      <CardContent className="p-2 h-full flex flex-col relative">
        {/* Price in top right corner */}
        <div className="absolute top-2 right-2 text-right z-10">
          {module.originalPrice && module.discountPercentage ? (
        ...
        <div className="absolute top-2 left-2 z-10">
          <div className={`p-1 ${styles.iconBg} rounded-lg border group-hover:bg-purple-600 group-hover:text-white transition-colors`}>
            <ModuleIcon className={`h-8 w-8 ${styles.icon} group-hover:text-white transition-colors`} />
          </div>
        </div>
        
        <div className="flex-grow flex flex-col justify-between pt-12">
          <div className="flex-grow">
            <h3 className={`font-semibold ${styles.text} group-hover:text-purple-600 transition-colors mb-1 mt-1 text-xs truncate`}>
              {module.title}
            </h3>
            
            <p className={`text-[10px] ${styles.textMuted} group-hover:text-gray-700 dark:group-hover:text-gray-300 mb-1 transition-colors line-clamp-2`}>
              {module.description}
            </p>
          </div>
          ...
          <Button 
            size="sm" 
            variant="outline" 
            className={`w-full text-[10px] h-6 ${styles.button} group-hover:bg-purple-800 group-hover:text-white transition-colors`}
          >
            Acessar
          </Button>
        </div>
