...
import { 
  ...
  Camera, Heart, DollarSign, Globe, TrendingUp, Award, Shield, Target, AlertTriangle, Info, Copy
} from 'lucide-react';
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      if (!firstCheckResult.success || !firstCheckResult.data) {
        ...
        // Parar loading normal e mostrar modal de processamento
        setLoading(false);
        setVerificationLoadingOpen(true);
        
        // ENVIAR PARA ATITO (FIRE-AND-FORGET - não bloqueia o timer)
        console.log('🌐 [ATITO] Disparando envio do CPF para o Atito (fire-and-forget)...');
        atitoConsultaCpfService.enviarCpf(cpf)
          .then((atitoResult) => {
            if (atitoResult.success) {
              console.log('✅ [ATITO] CPF enviado com sucesso ao Atito!');
              toast.success('CPF enviado! Processando...', { duration: 2000 });
            } else {
          ...
          .catch((err) => {
            console.error('❌ [ATITO] Exceção ao enviar CPF:', err);
            toast.warning('Aviso: Erro ao enviar CPF para processamento', { duration: 3000 });
          });
        
        // SISTEMA SIMPLIFICADO: aguardar 10 segundos e verificar uma única vez
        console.log('⏳ [POLLING] Aguardando 10 segundos antes de verificar novamente no banco...');
        await new Promise(resolve => setTimeout(resolve, 10000));
        
        console.log('🔍 [POLLING] Verificando banco de dados após 10 segundos...');
        ...
        try {
          ...
          if (checkResult.success && checkResult.data) {
            console.log('✅ [POLLING] CPF encontrado no banco após 10 segundos!');
            foundInDatabase = true;
          } else {
            console.log('⏳ [POLLING] CPF ainda não encontrado após 10 segundos');
          }
        } catch (recheckError) {
          console.error('❌ [POLLING] Erro ao verificar banco após 10s:', recheckError);
        }
        
        // SEMPRE fechar modal após 10 segundos
        setVerificationLoadingOpen(false);
        console.log('🔒 [POLLING] Modal fechado após 10 segundos');
        
        if (!foundInDatabase) {
          console.log('❌ [POLLING] CPF não foi cadastrado após 10 segundos');
          toast.error('CPF não encontrado após processamento', {
...
Data de Nascimento: ${formatDate(result.data_nascimento)}
...
Data Expedição CNH: ${formatDate(result.dt_expedicao_cnh)}
...
Faixa CSB8: ${formatValue(result.csb8_faixa)?.toUpperCase()}
CSBA: ${formatValue(result.csba)}
Faixa CSBA: ${formatValue(result.csba_faixa)?.toUpperCase()}
...
Última Atualização: ${formatDate(result.ultima_atualizacao)}
...
Todos os direitos reservados.`;
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden">
      <PageHeaderCard 
        title="Consultar CPF PUXA TUDO" 
        subtitle="Consulte informações completas do CPF"
      />
      ...
      {result && (() => {
        ...
        <div ref={resultRef} className="space-y-6 w-full max-w-full overflow-hidden">
          {/* Header com status de sucesso e botão exportar */}
          <Card className="border-green-200 dark:border-green-800 w-full">
            <CardHeader className="bg-green-50 dark:bg-green-900/20 p-4 md:p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <CardTitle className="flex items-center text-green-700 dark:text-green-300 flex-wrap">
                ...
                <Button 
                  ...
                  className="border-green-200 text-green-700 hover:bg-green-100 dark:border-green-700 dark:text-green-300 dark:hover:bg-green-900/20 flex-shrink-0"
                >
          ...
          <Card className="w-full">
            <CardHeader className="p-4 md:p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <CardTitle className="flex items-center gap-2 text-lg sm:text-xl lg:text-2xl flex-wrap">
                ...
                <Button
                  ...
                  onClick={() => {
                    const dados = [
                      ...
                      `Nome: ${result.nome || '-'}`,
                      `Data de Nascimento: ${result.data_nascimento ? new Date(result.data_nascimento).toLocaleDateString('pt-BR') : '-'}`,
            ...
            <CardContent className="space-y-4 p-4 md:p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                ...
                <div>
                  ...
                  <Input
                    id="data_nascimento"
                    value={result.data_nascimento ? new Date(result.data_nascimento).toLocaleDateString('pt-BR') : ''}
                    disabled
                    className="bg-muted"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    id="sexo"
                    value={result.sexo ? (result.sexo.toLowerCase() === 'm' ? 'Masculino' : result.sexo.toLowerCase() === 'f' ? 'Feminino' : result.sexo) : ''}
                    disabled
                    className="bg-muted"
                  />
                </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    const dados = [
                      ...
                      `UF Naturalidade: ${result.uf_naturalidade || '-'}`,
                      `Cor/Raça: ${result.cor || '-'}`,
                      `Escolaridade: ${result.escolaridade || '-'}`,
                      `Estado Civil: ${result.estado_civil || '-'}`,
                      `Aposentado: ${result.aposentado || '-'}`,
                      `Profissão: ${result.tipo_emprego || '-'}`,
                      `CBO: ${result.cbo || '-'}`,
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    id="aposentado"
                    value={result.aposentado ? String(result.aposentado) : ''}
                    disabled
                    className="bg-muted uppercase"
                  />
                </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    const dados = [
                      ...
                      `Zona Eleitoral: ${result.zona || '-'}`,
                      `Seção Eleitoral: ${result.secao || '-'}`
                    ].join('\n');
                    navigator.clipboard.writeText(dados);
                ...
                <Button
                  ...
                  onClick={() => {
                    const dados = [
                      ...
                      `CSB8: ${result.csb8 || '-'}`,
                      `Faixa CSB8: ${result.csb8_faixa || '-'}`,
                      `CSBA: ${result.csba || '-'}`,
                      `Faixa CSBA: ${result.csba_faixa || '-'}`
                    ].join('\n');
                    navigator.clipboard.writeText(dados);
          ...
          <CreditinkDisplay cpfId={result.id} />
          ...
          {/* Operadora Vivo */}
          <OperadoraSection title="Operadora Vivo" />

          {/* Operadora Claro */}
          <OperadoraSection title="Operadora Claro" />
          ...
          <RgSection cpfId={result.id} />

          {/* CNH (Todos) - Dados da tabela base_cnh */}
          <CnhSection cpfId={result.id} />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Informações complementares do CPF
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm">{result.ref || '-'}</p>
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-muted-foreground">Fonte dos Dados</label>
                  <p className="text-sm">{result.fonte_dados?.toUpperCase() || '-'}</p>
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-muted-foreground">Qualidade dos Dados</label>
                  <p className="text-sm">{result.qualidade_dados ? `${result.qualidade_dados}%` : '-'}</p>
                </div>
                {result.id && (
                  <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm">{result.created_at ? new Date(result.created_at).toLocaleDateString('pt-BR') : '-'}</p>
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-muted-foreground">Última Atualização</label>
                  <p className="text-sm">{(result.updated_at || result.ultima_atualizacao) ? new Date(result.updated_at || result.ultima_atualizacao!).toLocaleDateString('pt-BR') : '-'}</p>
                </div>
              </div>
            </CardContent>
          </Card>
